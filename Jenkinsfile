...
pipeline {
    ...
    stages {
        ...
        stage('Deploy on WSL') {
            steps {
                echo 'Copying JAR to WSL shared folder for manual deployment...'
                bat '''
                if not exist C:\\springboot-app mkdir C:\\springboot-app
                copy target\\forinterviewpracticespringbootalltopicimplementaion-0.0.1-SNAPSHOT.jar C:\\springboot-app\\
                echo "JAR copied to /mnt/c/springboot-app/. You can now run it manually in WSL."
                '''
            }
        }
    }
    ...
    post {
        success {
            echo '✅ Build completed. JAR deployed to Windows (8080). Ready for manual run on WSL (8081, 8082).'
        }
        failure {
            echo '❌ Deployment failed.'
        }
